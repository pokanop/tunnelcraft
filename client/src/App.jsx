    ...
    <div className="wrap">
      ...
      {cur.lesson && (
        <article className="lesson">
          <div className="lesson-est">
            {cur.lesson.est} · lesson {tIdx + 1} of {mod.lessons.length}
          </div>
          ...
          <div className="exrow">
            {prog.les[cur.lesson.id] ? (
              ...
              <button className="btn" onClick={() => markDone(cur.lesson.id)}>
                MARK COMPLETE →
              </button>
            )}
          </div>
      ...
      <div className="navrow">
        <button
          ...
          onClick={() => {
            tIdx > 0 ? setTab(tabs[tIdx - 1].key) : go({ v: "mod", id: ALL_MODULES[mIdx - 1].id });
          }}
        >
          ← PREV
        </button>
        {tIdx < tabs.length - 1 ? (
          <button className="btn ghost" onClick={() => setTab(tabs[tIdx + 1].key)}>
            NEXT →
          </button>
        ) : mIdx < ALL_MODULES.length - 1 ? (
          <button className="btn" onClick={() => go({ v: "mod", id: ALL_MODULES[mIdx + 1].id })}>
            NEXT MODULE: {ALL_MODULES[mIdx + 1].code} →
          </button>
        ) : (
...
export default function App() {
  ...
  const pushSoon = () => {
    ...
    timer.current = setTimeout(async () => {
      try {
        ...
        setSync(e.status === 401 ? "local" : "offline");
  ...
  const reviewAnswer = (key, correct) =>
    update((p) => {
