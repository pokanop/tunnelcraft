...
export function AccountView({ user, onSignOut, onDeleted, onBack }) {
  ...
  const refresh = () => api.sessions().then((r) => setSessions(r.sessions)).catch(() => setSessions([]));
  useEffect(() => { refresh(); }, []);
  ...
  const deleteAcct = () => {
    if (!delArm) { setDelArm(true); setTimeout(() => setDelArm(false), 5000); return; }
    run(async () => { await api.deleteAccount(delPw); onDeleted(); });
  };
      ...
      <div className="authcard acctcard">
        ...
        <div className="acctsec">
              ...
              {sessions.map((s) => (
                <li className="sessrow" key={s.handle}>
                  <div className="sess-info">
                    <div className="sess-ua" title={s.userAgent}>{s.current ? "● THIS DEVICE — " : ""}{s.device}</div>
                    <div className="sess-meta">last seen {s.lastSeen} · signed in {s.createdAt}</div>
                  </div>
                  {!s.current && (
                    <button
                      ...
                      onClick={() => run(async () => { await api.revokeSession(s.handle); refresh(); }, "Signed out " + s.device)}
                    >SIGN OUT</button>
                  )}
                </li>
              ))}
          ...
          <div className="authactions">
            <button className="btn ghost" disabled={busy} onClick={() => run(async () => { await api.logout(); onSignOut(); })}>SIGN OUT</button>
            <button className="btn ghost" disabled={busy} onClick={logoutAll}>SIGN OUT EVERYWHERE</button>
          </div>
        </div>
        ...
        <div className="acctsec">
          ...
          <div className="authfields">
            <div className="fld">
              ...
              <input type="password" value={curPw} autoComplete="current-password" onChange={(e) => setCurPw(e.target.value)} />
            </div>
            <div className="fld">
          ...
          <button className="btn" disabled={busy || newPw.length < 8} onClick={changePw}>UPDATE PASSWORD</button>
        </div>

        <div className="acctsec danger">
          <div className="acctsec-t">DANGER ZONE</div>
          <p className="authsub">Deleting your account permanently removes your progress and sign-ins. This cannot be undone.</p>
          <div className="authfields">
            <div className="fld">
        ...
        {msg && <div className="verdict good" role="status">✓ {msg}</div>}
...
export function ForgotForm({ onDone }) {
  ...
  const submit = async () => {
    ...
    catch { setSent(true); /* identical UX either way */ }
    finally { setBusy(false); }
  };
  return sent ? (
    <p className="authsub" role="status">If that email has an account, a reset link is on its way. Open it on this device to set a new password.</p>
  ) : (
    <>
      <div className="authfields">
        <div className="fld">
      ...
      <div className="authactions">
        <button className="btn" disabled={busy || !email.includes("@")} onClick={submit}>SEND RESET LINK</button>
...
export function ResetForm({ token, onDone }) {
  ...
  const submit = async () => {
    setErr(""); setBusy(true);
    try { await api.resetPassword(token, pw); setOk(true); }
    catch (e) { setErr(e.message); }
    finally { setBusy(false); }
  };
  return ok ? (
    <>
      <p className="authsub" role="status">Password updated — every existing session was signed out for safety. Sign in with your new password.</p>
    ...
    <>
      ...
      {err && <div className="verdict badv" role="alert">✗ {err}</div>}
      <div className="authactions">
        <button className="btn" disabled={busy || pw.length < 8} onClick={submit}>SET NEW PASSWORD</button>
      </div>
    </>
  );
}
