...
/* ---------------- N01 ---------------- */
{
  id: "m01", code: "N01", title: "The Map: How Networks Actually Move Bytes",
  layers: ["L1","L2","L3","L4","L7"], est: "~75 min",
  tag: "Layers, encapsulation, IP addressing, routing, and MTU — the mental model every tunnel is built on.",
  lessons: [
    { id: "m01l1", title: "Why layers exist (and which ones matter)", est: "8 min", blocks: [
      ...
      { code: { lang: "text", body:
`A WireGuard-tunneled web request, on the wire:
...
The outer stack exists only to carry it across the underlay.` } },
      { note: "Vocabulary you will use daily: the **underlay** is the real network path (your Wi-Fi, your ISP). The **overlay** is the virtual network your tunnel creates. Every hard VPN bug is a disagreement between the two.", label: "FIELD NOTE" },
    ]},
    { id: "m01l2", title: "IP addressing & CIDR, cold", est: "12 min", blocks: [
...
// and of your AllowedIPs matcher.` } },
    ...
    { id: "m01l3", title: "Routing: longest prefix wins", est: "10 min", blocks: [
...
192.168.1.0/24 dev wlan0 scope link      # the LAN` } },
      ...
      { note: "This /1 + /1 override plus the endpoint /32 pin is exactly what `wg-quick` does. When you build your own client you will re-implement it on every OS — with a different API each time (S01).", label: "FIELD NOTE" },
    ]},
    { id: "m01l4", title: "MTU, fragmentation & the 80-byte tax", est: "10 min", blocks: [
...
= safe inner MTU             1420` } },
      ...
      { note: "Debug ritual: `ping -M do -s 1392 peer` probes the path with don't-fragment set (1392 payload + 28 header = 1420 packet). Binary-search the size until it stops working — that is your real path MTU.", label: "DEBUG RITUAL" },
    ]},
  ],
  exercises: [
    { id: "m01e1", type: "order", title: "Encapsulation, in order", kind: "SEQUENCE LAB",
      prompt: "A browser sends an HTTP request. Tap the stages in the order they wrap the data on its way down the stack.",
      items: [
    ...
    { id: "m01e2", type: "cidr", title: "CIDR trainer — infinite reps", kind: "LIVE DRILL",
      prompt: "Subnet math must be reflexive before you write an AllowedIPs matcher. Compute the network address, broadcast address, and usable host count. New problems forever." },
  ],
  quiz: { id: "m01q", questions: [
    { q: "A client must reach its VPN server at 203.0.113.5 while routing all traffic through wg0. Why add a 203.0.113.5/32 route via the physical interface?",
      opts: ["To make the handshake faster by skipping encryption", "To prevent a routing loop where the tunnel's own encrypted packets are routed back into the tunnel", "Because /32 routes are required for UDP traffic"], a: 1,
    ...
    { q: "Your tunnel MTU equals the underlay's 1500. Ping works but HTTPS pages hang forever. Most likely cause?",
      opts: ["The AEAD cipher is too slow for large packets", "Large encapsulated packets exceed the underlay MTU and are black-holed", "The TCP handshake is blocked by a firewall"], a: 1,
    ...
    { q: "Which route wins for destination 10.8.0.7: 0.0.0.0/0 via eth0, 10.8.0.0/24 dev wg0, or 10.8.0.7/32 dev lo?",
      opts: ["The default route — it is listed first", "10.8.0.0/24 — it is the overlay subnet", "10.8.0.7/32 — longest prefix always wins"], a: 2,
...
/* ---------------- N08 ---------------- */
{
  id: "m02", code: "N08", title: "Transport: TCP, UDP & the Socket Contract",
  layers: ["L4"], est: "~55 min",
  tag: "Streams vs datagrams, the handshake, why serious tunnels ride UDP, and what a socket really promises.",
  lessons: [
    { id: "m02l1", title: "UDP: the honest datagram", est: "8 min", blocks: [
...
fn main() -> std::io::Result<()> {
    ...
    { id: "m02l2", title: "TCP: streams, the handshake, and the meltdown", est: "10 min", blocks: [
      ...
      { note: "Design rule: the carrier should be **dumber** than the cargo. TCP inside UDP: fine. TCP inside TCP: meltdown. That asymmetry shapes every protocol choice in this course.", label: "DESIGN RULE" },
    ]},
    { id: "m02l3", title: "Sockets: the API under every crate", est: "10 min", blocks: [
    ...
    { id: "m02l4", title: "Firewalls & conntrack: state you don't control", est: "8 min", blocks: [
  ...
  exercises: [
    { id: "m02e1", type: "order", title: "Life of a TCP connection", kind: "SEQUENCE LAB",
  ...
  quiz: { id: "m02q", questions: [
    { q: "Why do WireGuard, QUIC, and modern VPN protocols run over UDP rather than TCP?",
      opts: ["UDP packets are encrypted by default", "Tunneling reliable traffic over a reliable carrier causes competing retransmission loops (meltdown); UDP carries without interfering", "UDP has larger maximum packet sizes than TCP"], a: 1,
    ...
    { q: "You bind a UDP socket and call connect() on it. What actually happens?",
      opts: ["A three-way handshake runs with the peer", "Nothing on the wire — the kernel just fixes the default peer and filters inbound datagrams to it", "The socket switches to reliable delivery mode"], a: 1,
    ...
    { q: "An outbound UDP flow works, then replies silently stop arriving after ~30 seconds of quiet. The most likely culprit is:",
      opts: ["The remote host crashed", "A stateful firewall or NAT expired its mapping for the idle flow", "UDP checksum failures"], a: 1,
...
/* ---------------- N11 ---------------- */
{
  id: "m03", code: "N11", title: "The Hostile Internet: NAT & DNS",
  layers: ["L3","L4","L7"], est: "~55 min",
  tag: "Address translation, the four NAT temperaments, DNS resolution, and the leaks that make or break a VPN.",
  lessons: [
    { id: "m03l1", title: "NAT: the lie that saved IPv4", est: "8 min", blocks: [
...
reply to 203.0.113.9:41000 -> table lookup -> 192.168.1.7:51820` } },
    ...
    { id: "m03l2", title: "The four NAT temperaments", est: "10 min", blocks: [
      ...
      { tbl: { head: ["Type", "Behavior", "Hole-punchable?"], rows: [
        ["Full cone", "One mapping for all destinations; anyone may send to it", "Trivially"],
        ["Restricted cone", "One mapping; only IPs you contacted may reply", "Yes"],
        ["Port-restricted", "One mapping; only exact ip:port you contacted may reply", "Yes, with simultaneous send"],
        ["Symmetric", "New mapping per destination — external port unpredictable", "Only via port prediction or relay"],
      ]}},
    ...
    { id: "m03l3", title: "DNS: resolution, and where VPNs leak", est: "10 min", blocks: [
      ...
      { note: "Modern wrinkle: OS-level encrypted DNS (DoH/DoT) and browser-level DoH can bypass your carefully-set resolver entirely. A production client detects and handles this, or documents that it doesn't.", label: "FIELD NOTE" },
    ]},
    { id: "m03l4", title: "Keepalives: paying rent on middlebox state", est: "7 min", blocks: [
  ...
  exercises: [
    { id: "m03e1", type: "order", title: "Life of a NAT'd packet", kind: "SEQUENCE LAB",
  ...
  quiz: { id: "m03q", questions: [
    { q: "STUN reports your public endpoint as 203.0.113.9:41000, but a peer sending there gets nothing, and each new destination you contact sees a different port. Your NAT is:",
      opts: ["Full cone", "Port-restricted cone", "Symmetric"], a: 2,
    ...
    { q: "A VPN advertises full-tunnel encryption but the OS keeps using the DHCP-provided resolver. What happens?",
      opts: ["Nothing — DNS is already encrypted by default", "Every hostname the user visits is visible to the local network: a DNS leak", "DNS fails because the resolver is unreachable"], a: 1,
    ...
    { q: "Why is WireGuard's suggested persistent keepalive 25 seconds rather than 60?",
      opts: ["The protocol requires a handshake every 25 s", "Common NAT/firewall UDP timeouts start around 30 s; the keepalive must refresh state before the shortest lease expires", "Anything longer would trigger a rekey"], a: 1,
...
/* ---------------- R01 ---------------- */
{
  id: "m04", code: "R01", title: "Rust Foundations: Ownership as a Network Skill",
  layers: ["RS"], est: "~80 min",
  tag: "Ownership, borrowing, Result, traits, lifetimes — taught on packet buffers, because that's where you'll use them.",
  lessons: [
    { id: "m04l1", title: "Ownership & borrowing on packet buffers", est: "14 min", blocks: [
      ...
      { code: { lang: "rust", body:
`fn checksum(buf: &[u8]) -> u16 { /* read-only borrow */ 0 }
...
// just prevented a use-after-send bug.` } },
    ...
    { id: "m04l2", title: "Result, Option & the ? operator", est: "12 min", blocks: [
...
fn pump(sock: &UdpSocket, buf: &mut [u8]) -> Result<usize, TunnelError> {
    ...
    { id: "m04l3", title: "Traits: designing small contracts", est: "14 min", blocks: [
      ...
      { code: { lang: "rust", body:
`/// Anything that can carry an outbound IP packet.
trait PacketSink {
...
struct Router { engines: Vec<Box<dyn PacketSink>> }` } },
    ...
    { id: "m04l4", title: "Lifetimes without fear", est: "12 min", blocks: [
      ...
      { code: { lang: "rust", body:
`/// A parsed view — zero copies, just borrows into the datagram.
struct Ipv4View<'a> {
    header: &'a [u8],
    payload: &'a [u8],
}
...
// which is a use-after-free caught at build time.` } },
    ...
    { id: "m04l5", title: "Idiomatic patterns you'll reach for weekly", est: "12 min", blocks: [
...
`impl TryFrom<&[u8]> for PublicKey {
    ...
    fn try_from(b: &[u8]) -> Result<Self, KeyError> {
      ...
      { code: { lang: "rust", body:
`// find the best route: no bounds checks, no off-by-ones
let best = routes.iter()
  ...
  exercises: [
    { id: "m04e1", type: "blank", title: "Borrow-check the packet path", kind: "CODE LAB",
      prompt: "Fill each blank so this compiles — choose the least-privileged borrow that still works.",
      code:
`fn checksum(buf: §0§) -> u16 { 0 /* ... */ }
...
transmit(packet);              // hand it off for good`,
  ...
  quiz: { id: "m04q", questions: [
    { q: "Why does the compiler reject holding a shared reference to a packet buffer while another task mutates it?",
      opts: ["References are slower than copies", "Shared + exclusive access at once is the definition of a data race; Rust makes it unrepresentable", "Buffers must always be heap-allocated first"], a: 1,
    ...
    { q: "You're writing the reusable core library of your VPN client. Errors should be:",
      opts: ["anyhow::Result everywhere for convenience", "A concrete enum (e.g. with thiserror) so callers can match on variants and react differently", "Strings, logged and swallowed"], a: 1,
    ...
    { q: "A hot per-packet function needs polymorphism over the sink type. Prefer:",
      opts: ["Box<dyn PacketSink> for flexibility", "Generics with a trait bound — monomorphized, no indirection in the hot path", "Function pointers"], a: 1,
...
/* ---------------- R03 ---------------- */
{
  id: "m05", code: "R03", title: "Async Rust & Tokio: One Thread, Ten Thousand Sockets",
  layers: ["RS"], est: "~70 min",
  tag: "Futures, tasks, channels, select!, and cancellation — the concurrency toolkit a tunnel daemon runs on.",
  lessons: [
    { id: "m05l1", title: "Why async, and what a Future really is", est: "12 min", blocks: [
      ...
      { code: { lang: "rust", body:
`// This function does NOTHING until awaited.
async fn handshake(peer: SocketAddr) -> Result<Session, Error> {
    ...
    finalize(resp)
      ...
      { note: "Two laws of the runtime: never block a worker thread (no std::net, no heavy CPU loops — use `spawn_blocking` for those), and remember every `.await` is a possible pause point where your task's state must be self-contained.", label: "RUNTIME LAW" },
    ]},
    { id: "m05l2", title: "Tasks: spawning and the 'static bound", est: "12 min", blocks: [
      ...
      { code: { lang: "rust", body:
`let sock = Arc::new(UdpSocket::bind("0.0.0.0:51820").await?);
...
let recv_task = tokio::spawn(async move {
    ...
    { id: "m05l3", title: "Channels & the actor pattern", est: "12 min", blocks: [
...
async fn engine_actor(mut rx: mpsc::Receiver<Cmd>, mut state: EngineState) {
    while let Some(cmd) = rx.recv().await {
      ...
      { note: "This is the skeleton of a TunnelEngine runtime: the trait object lives inside an actor, the rest of the system holds only a cheap `mpsc::Sender` handle. Backpressure comes free — a bounded channel makes fast producers wait.", label: "ARCHITECTURE SEED" },
    ]},
    { id: "m05l4", title: "select!, timeouts & graceful cancellation", est: "12 min", blocks: [
...
`loop {
  ...
  exercises: [
    { id: "m05e1", type: "blank", title: "Wire up the pump task", kind: "CODE LAB",
      prompt: "Complete this receive-pump so datagrams flow from the socket into the engine's channel.",
      code:
`async fn pump(sock: Arc<UdpSocket>, tx: mpsc::Sender<Vec<u8>>) {
    let mut buf = vec![0u8; 65535];
    loop {
...
§2§(pump(sock, tx));`,
  ...
  quiz: { id: "m05q", questions: [
    { q: "Inside an async task you call std::net::UdpSocket::recv_from (the blocking one). What goes wrong?",
      opts: ["Nothing — tokio detects and adapts", "It blocks an entire runtime worker thread, starving every other task scheduled on it", "It panics immediately"], a: 1,
    ...
    { q: "A select! branch loses the race and its future is dropped halfway through reading a length-prefixed frame from a TCP stream. The risk is:",
      opts: ["None — futures are always cancel-safe", "Partial-read state is lost; the stream is now desynchronized (not cancel-safe)", "The TCP connection is automatically reset"], a: 1,
...
/* ---------------- R04 ---------------- */
{
  id: "m06", code: "R04", title: "Network Programming in Rust, for Real",
  layers: ["L4","RS"], est: "~65 min",
  tag: "Sockets in practice: buffers, framing, zero-copy Bytes, socket2 escape hatches, and typed config with serde.",
  lessons: [
    { id: "m06l1", title: "std::net vs tokio::net — and buffer discipline", est: "12 min", blocks: [
      ...
      { code: { lang: "rust", body:
`// One buffer, reused across the loop's lifetime.
let mut buf = vec![0u8; 65535];
loop {
    ...
    { id: "m06l2", title: "Framing: where streams become messages", est: "12 min", blocks: [
...
async fn read_frame(s: &mut TcpStream, max: u32) -> io::Result<Vec<u8>> {
    ...
    Ok(payload)
      ...
      { note: "That max check is not optional. A 4 GB length prefix from a malicious peer is a one-packet denial of service against naive code. Parsers face the internet; write them like it.", label: "SECURITY RULE" },
    ]},
    { id: "m06l3", title: "Zero-copy pipelines with Bytes", est: "12 min", blocks: [
...
telemetry_tx.send(pkt).await?;      // same bytes, zero copies` } },
    ...
    { id: "m06l4", title: "socket2: the platform escape hatch", est: "12 min", blocks: [
...
let sock = tokio::net::UdpSocket::from_std(raw.into())?;` } },
    ...
    { id: "m06l5", title: "Typed config with serde", est: "10 min", blocks: [
...
let cfg: PeerConfig = toml::from_str(raw)?;` } },
  ...
  exercises: [
    { id: "m06e1", type: "blank", title: "Harden the frame reader", kind: "CODE LAB",
      prompt: "Fill the blanks to make this TCP frame reader both correct and hostile-input-safe.",
      code:
`async fn read_frame(s: &mut TcpStream) -> io::Result<Vec<u8>> {
    let mut len_bytes = [0u8; 4];
  ...
  quiz: { id: "m06q", questions: [
    { q: "Your TCP-based control channel occasionally 'merges' two messages into one read. The bug is:",
      opts: ["A kernel buffer overflow", "Missing framing — TCP is a byte stream with no message boundaries, so reads and writes don't correspond", "Nagle's algorithm corrupting data"], a: 1,
    ...
    { q: "One received datagram must be seen by the router task and the metrics task. The zero-copy idiom is:",
      opts: ["Clone the Vec<u8> for each task", "Freeze it into Bytes and clone the handle — O(1) refcounted views", "Wrap the Vec in Mutex and share a reference"], a: 1,
    ...
    { q: "Why does a VPN client bind its underlay UDP socket to the physical interface (SO_BINDTODEVICE or equivalent)?",
      opts: ["It improves encryption strength", "So the tunnel's own encrypted packets can never be routed back into the tunnel once the default route points at it", "It is required to receive broadcast packets"], a: 1,
      why: "Belt to go with the /32-route suspenders: the loop-prevention problem from N01, solved at the socket layer." },
  ]},
},
{
  id: "m07", code: "T01", title: "TUN/TAP & Packet I/O", tag: "Where userspace touches raw packets",
  layers: ["L2","L3","RS"], est: "~70 min",
  lessons: [
    { id: "m07l1", title: "TUN vs TAP", est: "~10 min", blocks: [
      ...
      { note: "The entire trick of a VPN client is: get the OS to route interesting traffic to your TUN, read the packets, encrypt them, and send them out a normal UDP socket bound to the real interface. Everything else is bookkeeping.", label: "the one-sentence VPN" },
    ...
    { id: "m07l2", title: "Creating a TUN on each OS", est: "~10 min", blocks: [
    ...
    { id: "m07l3", title: "The packet loop", est: "~10 min", blocks: [
      ...
      { note: "The `select!` loop is also your integration point for everything from R03: config watch channels, cancellation tokens, and timer ticks all become just more arms.", label: "it all connects" },
    ]},
    { id: "m07l4", title: "smoltcp: a TCP/IP stack in your process", est: "~10 min", blocks: [
      ...
      { code: { lang: "rust", title: "smoltcp in one glance", body: "let mut iface = Interface::new(config, &mut device, Instant::now());\nlet mut sockets = SocketSet::new(vec![]);\nlet tcp = tcp::Socket::new(rx_buf, tx_buf);\nlet handle = sockets.add(tcp);\n\nloop {\n    iface.poll(Instant::now(), &mut device, &mut sockets);\n    let sock = sockets.get_mut::<tcp::Socket>(handle);\n    if sock.can_recv() { sock.recv(|data| { consume(data); (data.len(), ()) })?; }\n}" } },
    ]},
  ],
  exercises: [
    { id: "m07e1", kind: "ORDER LAB", title: "Life of an outbound packet", type: "order",
  ...
  quiz: { id: "m07q", questions: [
    { q: "Why do VPN clients use TUN rather than TAP?",
      opts: ["TAP is deprecated on modern kernels", "They route at L3, so raw IP packets are exactly what they need — Ethernet framing is dead weight", "TUN devices are faster because they are compiled into the kernel"], a: 1,
    ...
    { q: "What is the macOS utun quirk your read/write path must handle?",
      opts: ["Packets are big-endian only", "A 4-byte protocol-family header prefixes every packet", "Reads must be exactly MTU-sized"], a: 1,
    ...
    { q: "Why should your Rust core accept an already-open TUN device instead of always creating one?",
      opts: ["Creating devices is slow", "iOS and Android never let you create the device — the OS hands your code a pre-configured fd", "It avoids needing the tun crate"], a: 1,
...
{
  id: "m08", code: "T02", title: "Crypto for Tunnels", tag: "Noise, WireGuard's handshake, and key lifetimes",
  layers: ["L7","RS"], est: "~65 min",
  lessons: [
    { id: "m08l1", title: "The toolbox", est: "~10 min", blocks: [
      ...
      { note: "AEAD nonces must NEVER repeat under the same key. Encrypt two messages with one (key, nonce) pair and ChaCha20-Poly1305 loses both confidentiality and integrity. WireGuard's fix is elegant: the message counter IS the nonce — replay protection and nonce uniqueness from one integer.", label: "the cardinal sin" },
    ...
    { id: "m08l2", title: "Noise: handshakes from a pattern language", est: "~10 min", blocks: [
    ...
    { id: "m08l3", title: "The WireGuard handshake, message by message", est: "~10 min", blocks: [
      ...
      { note: "This 3-step dance is exactly what the animated strip on this app's home screen depicts: initiation → response → transport.", label: "you have seen this" },
    ]},
    { id: "m08l4", title: "Sessions age fast on purpose", est: "~10 min", blocks: [
  ...
  quiz: { id: "m08q", questions: [
    { q: "Why can WireGuard complete its handshake in one round trip?",
      opts: ["It skips authentication on the first message", "Noise_IK: the initiator already knows the responder's static public key from config", "It reuses keys from the previous session"], a: 1,
    ...
    { q: "What makes nonce reuse catastrophic with ChaCha20-Poly1305?",
      opts: ["It slows down encryption", "Same (key, nonce) on two messages breaks both confidentiality and authenticity", "It causes packet loss"], a: 1,
    ...
    { q: "A port scanner probes a WireGuard endpoint. What does it see?",
      opts: ["An ICMP port-unreachable reply", "Nothing — unauthenticated packets get no response at all", "A TCP RST"], a: 1,
      why: "Silence discipline: if the MAC doesn't verify, the packet never existed. The service is invisible without the right key." },
  ]},
},
{
  id: "m09", code: "T03", title: "WireGuard in Practice", tag: "Cryptokey routing, config anatomy, and boringtun",
  layers: ["L3","L7","RS"], est: "~65 min",
  lessons: [
    { id: "m09l1", title: "Cryptokey routing: the big idea", est: "~10 min", blocks: [
    ...
    { id: "m09l3", title: "boringtun: WireGuard as a library", est: "~10 min", blocks: [
      ...
      { code: { lang: "rust", title: "the TunnResult contract", body: "match tunn.decapsulate(None, &ciphertext, &mut buf) {\n    TunnResult::WriteToTunnelV4(pkt, _src) => tun.send(pkt).await?,\n    TunnResult::WriteToNetwork(resp) => {\n        // handshake reply or queued packet — send it, then drain:\n        udp.send_to(resp, endpoint).await?;\n        while let TunnResult::WriteToNetwork(more) =\n            tunn.decapsulate(None, &[], &mut buf) {\n            udp.send_to(more, endpoint).await?;\n        }\n    }\n    TunnResult::Done => {}\n    TunnResult::Err(e) => log_drop(e),\n    _ => {}\n}" } },
      { note: "The drain loop is the classic boringtun gotcha: after a handshake completes, queued packets are released one call at a time. Call decapsulate with an empty datagram until it stops returning WriteToNetwork, or your first packets after every handshake silently stall.", label: "gotcha" },
    ]},
    { id: "m09l4", title: "Driving the timers", est: "~10 min", blocks: [
      ...
      { code: { lang: "rust", title: "the timer arm of your select! loop", body: "let mut tick = tokio::time::interval(Duration::from_millis(250));\nloop {\n    tokio::select! {\n        _ = tick.tick() => {\n            for peer in peers.iter_mut() {\n                match peer.tunn.update_timers(&mut buf) {\n                    TunnResult::WriteToNetwork(pkt) =>\n                        udp.send_to(pkt, peer.endpoint).await?,\n                    _ => {}\n                }\n            }\n        }\n        // ... tun + udp arms from T01 ...\n    }\n}" } },
  ...
  exercises: [
    { id: "m09e1", kind: "CODE LAB", title: "Complete the boringtun loop", type: "blank",
  ...
  quiz: { id: "m09q", questions: [
    { q: "A packet decrypts successfully under peer X's key, but its inner source IP is not in X's AllowedIPs. What happens?",
      opts: ["It is forwarded anyway since decryption proves authenticity", "It is dropped — cryptokey routing binds identity to addresses in both directions", "It is rerouted to the peer that owns that IP"], a: 1,
    ...
    { q: "Why does boringtun require you to call update_timers() yourself?",
      opts: ["A bug that was never fixed", "It's sans-I/O by design: it owns no clock, sockets, or threads — you drive it, which is why it embeds anywhere", "Only needed on Windows"], a: 1,
    ...
    { q: "Your phone switches from Wi-Fi to LTE mid-session. Why does the tunnel survive?",
      opts: ["The OS replays the handshake automatically", "Peers are identified by public key, not address — the endpoint just updates when authenticated packets arrive from the new address", "TCP retransmits cover the gap"], a: 1,
      why: "Roaming is free when identity lives in the key. Compare IPsec, where an address change is an incident." },
  ]},
},
{
  id: "m10", code: "T04", title: "NAT Traversal", tag: "STUN, hole punching, ICE, and relays of last resort",
  layers: ["L3","L4"], est: "~60 min",
  lessons: [
    { id: "m10l1", title: "STUN: learning your own address", est: "~10 min", blocks: [
      ...
      { note: "Crucial detail: STUN must be sent from the SAME socket you'll use for tunnel traffic. A different socket gets a different mapping, and the address you learned is useless.", label: "same socket!" },
    ]},
    { id: "m10l2", title: "Hole punching", est: "~10 min", blocks: [
    ...
    { id: "m10l3", title: "ICE: try everything, pick the best", est: "~10 min", blocks: [
    ...
    { id: "m10l4", title: "Relays: TURN and DERP", est: "~10 min", blocks: [
      ...
      { note: "Reading list: Tailscale's 'How NAT traversal works' is the field's best essay. RFC 8445 (ICE), RFC 5389/8489 (STUN), RFC 8656 (TURN) are the formal versions.", label: "go deeper" },
    ]},
  ],
  exercises: [
    { id: "m10e1", kind: "ORDER LAB", title: "Two peers, two NATs, one connection", type: "order",
  ...
  quiz: { id: "m10q", questions: [
    { q: "Why must STUN queries use the same socket as tunnel traffic?",
      opts: ["STUN servers rate-limit new sockets", "NAT mappings are per-socket-flow — a different socket owns a different mapping, so the learned address wouldn't be yours", "The OS requires it"], a: 1,
    ...
    { q: "Why does simultaneous sending open both NATs?",
      opts: ["The packets merge in transit", "Each side's outbound packet creates a mapping, so the other side's packet arrives looking like a reply to an established flow", "NATs whitelist STUN traffic"], a: 1,
    ...
    { q: "What makes DERP-style relays acceptable to route through even though you don't trust them?",
      opts: ["They are audited annually", "Packets are already end-to-end encrypted by WireGuard — relays see only ciphertext and metadata", "They only relay handshakes"], a: 1,
      why: "Encrypt end-to-end first, then any relay is just an untrusted bent pipe. Design order matters." },
  ]},
},
{
  id: "m11", code: "S01", title: "Cross-Platform Engineering", tag: "One Rust core, five operating systems",
  layers: ["XP","L3","RS"], est: "~60 min",
  lessons: [
    { id: "m11l1", title: "The capability matrix", est: "~10 min", blocks: [
      ...
      { tbl: { head: ["Platform", "Packet device", "Routes & DNS", "Firewall / enforcement", "Privilege model"], rows: [
        ["Linux", "/dev/net/tun (ioctl)", "netlink (rtnetlink)", "nftables / iptables", "root or CAP_NET_ADMIN"],
        ["macOS", "utun control socket", "PF_ROUTE + scoped resolvers", "PF; NetworkExtension entitlements", "Network Extension (sandboxed)"],
        ["iOS", "handed to you", "NEPacketTunnelProvider settings object", "OS-enforced includeAllNetworks", "NEPacketTunnelProvider only"],
        ["Windows", "wintun rings", "IP Helper API + interface metrics", "WFP filters", "Administrator / service"],
        ["Android", "handed to you", "VpnService.Builder", "OS per-app VPN, protect()", "VpnService + user consent"],
      ]}},
    ...
    { id: "m11l2", title: "Routing & DNS, per platform", est: "~10 min", blocks: [
    ...
    { id: "m11l3", title: "Core/shell: the architecture that ships", est: "~10 min", blocks: [
      ...
      { note: "Threading gotcha: mobile shells call you on *their* threads (main thread, binder threads). The FFI layer's first job is to hop onto your tokio runtime (Handle::spawn) and never block the caller. Most 'random ANR/crash' bugs in Rust-core mobile apps are violations of this rule.", label: "FFI reality" },
    ]},
    { id: "m11l4", title: "Kill switch: fail closed", est: "~10 min", blocks: [
  ...
  quiz: { id: "m11q", questions: [
    { q: "Your Android build works, but the underlay UDP socket's packets keep getting routed back into the VPN. What's the platform-correct fix?",
      opts: ["Add a /32 route via the physical gateway", "Call VpnService.protect() on the socket fd", "Lower the tunnel interface metric"], a: 1,
    ...
    { q: "On Windows, DNS queries leak out the physical adapter even with tunnel DNS configured. Likely culprit?",
      opts: ["wintun dropped the packets", "Smart multi-homed name resolution racing queries across all adapters", "The DNS server is down"], a: 1,
    ...
    { q: "Why do serious clients put the kill switch in kernel firewall rules rather than in their own packet loop?",
      opts: ["Kernel rules are faster", "The rules must keep enforcing when the client process crashes — a kill switch that dies with your daemon is not a kill switch", "Firewall APIs are simpler"], a: 1,
    ...
    { q: "What belongs in the Rust core vs the native shell?",
      opts: ["Core: UI and permissions; shell: crypto", "Core: protocol, crypto, packet loop, state; shell: device creation, permissions, lifecycle, UI", "Everything in the core, always"], a: 1,
...
{
  id: "m12", code: "S02", title: "Architecture of a Real Client", tag: "Flows, engines, carriages, config, and posture",
  layers: ["XP","L7","RS"], est: "~75 min",
  lessons: [
    { id: "m12l1", title: "The unified Flow", est: "~10 min", blocks: [
      ...
      { code: { lang: "rust", title: "the currency", body: "pub struct Flow {\n    pub five_tuple: FiveTuple,\n    pub ingress: Ingress,        // Tun | Socks | Proxy | App\n    pub app: Option<AppId>,\n    pub opened_at: Instant,\n    pub posture: PostureSnapshot,\n}" } },
    ]},
    { id: "m12l2", title: "TunnelEngine and the Carriage", est: "~10 min", blocks: [
    ...
    { id: "m12l3", title: "ConfigSource as anti-corruption boundary", est: "~10 min", blocks: [
    ...
    { id: "m12l5", title: "Why MASQUE / CONNECT-IP exists", est: "~10 min", blocks: [
  ...
  exercises: [
    { id: "m12e1", kind: "ORDER LAB", title: "A Flow crosses the dispatch core", type: "order",
  ...
  quiz: { id: "m12q", questions: [
    { q: "What problem does normalizing everything into a Flow solve?",
      opts: ["It compresses packets", "It prevents each ingress type from growing its own routing/telemetry pipeline — one dispatch core, one set of rules, one place to test", "It is required by RFC 9484"], a: 1,
    ...
    { q: "Why model Carriage (Packet/Stream/Datagram) explicitly instead of a bytes-in-bytes-out engine API?",
      opts: ["Rust requires it", "Shape mismatches then become explicit, testable adapters (like smoltcp) instead of leaky assumptions buried in every engine", "It reduces memory use"], a: 1,
    ...
    { q: "Under NIST SP 800-207, what should a posture change (e.g., disk encryption disabled) do to already-established flows?",
      opts: ["Nothing — posture is checked at connect time", "Trigger re-evaluation: continuous verification means live flows can be downgraded or severed", "Only log a warning"], a: 1,
    ...
    { q: "Why carry IP packets in QUIC DATAGRAM frames (RFC 9221) rather than QUIC streams in MASQUE?",
      opts: ["Streams are encrypted differently", "Streams retransmit — putting TCP-carrying packets on a reliable layer recreates the TCP-over-TCP meltdown", "DATAGRAM frames are larger"], a: 1,
      why: "N08's lesson, resurfacing at the cutting edge: let the inner protocol own reliability." },
  ]},
},
{
  id: "m13", code: "S03", title: "Capstone Roadmap", tag: "From hello-tunnel to production client",
  layers: ["XP","RS","L7"], est: "~45 min",
  lessons: [
    { id: "m13l1", title: "The milestone ladder", est: "~10 min", blocks: [
    ...
    { id: "m13l2", title: "Testing tunnels honestly", est: "~10 min", blocks: [
      ...
      { note: "The netns + netem + tcpdump trio replaces a rack of test hardware. Mastering it is a career skill beyond VPNs.", label: "force multiplier" },
    ]},
    { id: "m13l3", title: "Shipping it", est: "~10 min", blocks: [
  ...
  exercises: [
    { id: "m13e1", kind: "CAPSTONE", title: "The builder's checklist", type: "check",
      prompt: "Track your ladder. Each item is a runnable artifact — check it off when yours runs.",
      items: [
