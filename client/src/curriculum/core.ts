...
assert_eq!(resp.§2§(), MessageClass::SuccessResponse);`,
  ...
  },
  {
    ...
    lessons: [
        ...
        blocks: [
          ...
          },
          {
            ul: [
              ...
              "**R11 — Peer-info exchange** (T05): protobuf peer records over the R2 framed protocol; 3+ nodes gossip and merge tables (see T05 peer-exchange sequence lab).",
              "**R12 — Relay-through-peer** (T05): when direct hole punching fails, route via an intermediate peer — `petgraph` path selection + forwarding (simulated blocked pair in netns).",
              "**R13 — Subnet proxy** (T05): advertise a LAN `/24` into the mesh; all peers receive and install the route automatically (site-to-site pattern).",
              "**R14 — UPnP port mapping** (T04/T06): integrate `igd-next` so the node opens a UDP port on the local router as a traversal aid — companion to R12's relay fallback.",
            ],
          },
        ],
      },
      {
        id: "m13l1b",
        title: "Mesh rungs: from peer tables to UPnP",
        est: "~12 min",
        blocks: [
          {
          ...
          {
            p: "**R13** extends the protobuf record's `routes` field: one node attached to a LAN advertises `192.168.1.0/24`; after gossip every peer installs that prefix into its overlay route table — the site-to-site pattern EasyTier and Tailscale subnet routers expose, but here you own the propagation logic.",
          },
          {
    ...
    exercises: [
      {
        ...
        items: [
          ...
          "Peer-info exchange: 3+ nodes gossip protobuf records over framed TCP and converge on one peer table",
          "Relay-through-peer: blocked direct path routes via intermediate peer (netns or simulated graph test)",
          "Subnet proxy: advertised /24 propagates to all peers in a 3-node setup",
          "UPnP: igd-next opens and verifies a UDP port mapping on the local router (or mock gateway in CI)",
        ],
      },
      {
        id: "m13e2",
        kind: "CODE LAB",
        title: "Merge a gossiped peer record (R11)",
        type: "blank",
        prompt:
          "Complete the receive path from T05's peer-exchange protocol: decode a framed protobuf record and merge by sequence.",
        code: "let (payload, _) = decode_frame(&wire)?;\nlet incoming = PeerRecord::decode(payload)?;\n\nif let Some(existing) = table.records.iter_mut()\n    .find(|r| r.origin_key == incoming.origin_key)\n{\n    if incoming.seq §0§ existing.seq {\n        *existing = incoming;\n    }\n} else {\n    table.records.§1§(incoming);\n}\n// shipped inside the R2 frame from lesson R04; wire format from T05 sequence lab",
        blanks: [
          { opts: [">", ">=", "<"], a: 0 },
          { opts: ["push", "pop", "clear"], a: 0 },
        ],
        why: "Highest sequence wins per origin — the merge rule from T05L3. New origins are inserted; stale records are ignored without error.",
      },
      {
        id: "m13e3",
        kind: "CODE LAB",
        title: "Pick the relay path (R12)",
        type: "blank",
        prompt:
          "When the direct A↔D link failed hole punching, fill the blanks so overlay routing returns the relay hop list.",
        code: 'use petgraph::algo::astar;\nuse petgraph::visit::EdgeRef;\n\n// Direct A-D edge removed — only A-C and C-D remain.\nlet best = astar(\n    &mesh,\n    node_a,\n    |n| n == node_d,\n    |e| *e.§0§(),\n    |_| 0,\n);\nlet (latency, path) = best.§1§("D unreachable");\n// path.len() == 3 => relay via C',
        blanks: [
          { opts: ["weight", "id", "index"], a: 0 },
          { opts: ["expect", "unwrap_or", "len"], a: 0 },
        ],
        why: "Edge weight is live latency (T05L4). `astar` returns the hop list — two overlay hops means relay-through-peer for free.",
      },
      {
        id: "m13e4",
        kind: "SEQUENCE LAB",
        title: "Site-to-site route propagation (R13)",
        type: "order",
        prompt:
          "Order the steps when node A, attached to 192.168.1.0/24, advertises that LAN into the mesh.",
        items: [
          "A adds 192.168.1.0/24 to its own PeerRecord.routes field",
          "A gossips the signed, sequenced record to peers B and C over framed TCP",
          "B and C merge the record (highest seq per origin) into their peer tables",
          "Each peer rebuilds its overlay RouteTable from all known PeerRecords",
          "Traffic destined for 192.168.1.0/24 is forwarded via A's tunnel endpoint",
        ],
        why: "Advertise → gossip → merge → install → forward. Same soft-state pattern as peer endpoints, but for prefixes — the site-to-site pattern.",
      },
      {
        id: "m13e5",
        kind: "CODE LAB",
        title: "Open a UPnP traversal port (R14)",
        type: "blank",
        prompt:
          "Fill the blanks in this igd-next sketch so the node maps its tunnel UDP port on the local router.",
        code: 'use igd_next::aio::tokio::Gateway;\nuse igd_next::PortMappingProtocol;\n\nlet gateway = Gateway::§0§().await?;\nlet external = gateway\n    .add_port(\n        PortMappingProtocol::§1§,\n        local_port,\n        local_port,\n        lease_secs,\n        "tunnelcraft",\n    )\n    .await?;\n// external is the router\'s public ip:port — share via peer record if useful',
        blanks: [
          { opts: ["discover", "connect", "bind"], a: 0 },
          { opts: ["UDP", "TCP", "BOTH"], a: 0 },
        ],
        why: "discover() finds the IGD on the LAN; WireGuard-shaped tunnels need UDP. Lease + description let the router reap stale mappings.",
      },
    ],
  },
];
