...
export const NET_B = [
{
  id: "n07", code: "N09", title: "TCP Deep Dive",
  layers: ["L4"], est: "~70 min",
  tag: "The state machine, the reliability machinery, and the two windows that decide how fast anything downloads.",
  lessons: [
    { id: "n07l1", title: "The state machine, honestly", est: "~10 min", blocks: [
    ...
    { id: "n07l2", title: "Reliability machinery", est: "~10 min", blocks: [
    ...
    { id: "n07l3", title: "Flow control: the receiver's window", est: "~10 min", blocks: [
      ...
      { note: "throughput ≤ window / RTT is a formula worth memorizing: it converts 'the download is slow' into arithmetic. 64 KB window at 80 ms RTT = 6.5 Mbps ceiling. On a gigabit link.", label: "the ceiling formula" },
    ]},
    { id: "n07l4", title: "Congestion control: the network's window", est: "~12 min", blocks: [
  ...
  exercises: [
    { id: "n07e1", type: "order", title: "One byte's guarantee", kind: "SEQUENCE LAB",
      prompt: "Segment 5 of a transfer is lost in transit. Order how TCP recovers — the fast path, no timeout.",
      items: [
    ...
    { id: "n07e2", type: "match", title: "Socket states as symptoms", kind: "MATCH LAB",
      prompt: "You run ss on a misbehaving box. Match each observation to its meaning.",
      pairs: [
        { t: "Thousands in TIME_WAIT", d: "Normal on a busy server — the closer's mandatory quarantine, not a leak" },
        { t: "Connect returns 'refused' instantly", d: "Host reachable, port closed — an RST came back" },
        { t: "Connect hangs, then times out", d: "A firewall is silently dropping packets — nothing answered at all" },
        { t: "Pile-up in SYN_RECV", d: "Half-open connections — likely a SYN flood or a broken path back to clients" },
        { t: "Stuck in CLOSE_WAIT for hours", d: "The peer closed; your application never called close() — an app bug, not a network one" },
      ],
  ...
  quiz: { id: "n07q", questions: [
    { q: "A transfer between continents crawls at ~6 Mbps on gigabit links with no loss. First suspect?",
      opts: ["Encryption overhead", "Window too small for the bandwidth-delay product — throughput is capped at window/RTT", "DNS latency"], a: 1,
    ...
    { q: "Why does the first closer sit in TIME_WAIT for a minute?",
      opts: ["To let the app finish reading", "To absorb stray old-connection packets so they can't corrupt a new connection on the same port pair", "Kernel garbage collection"], a: 1,
    ...
    { q: "During a big upload, the whole household's latency jumps to 2+ seconds. The mechanism?",
      opts: ["ISP throttling", "Bufferbloat: oversized queues delay the loss signal, so loss-based TCP keeps the buffer full and everything queues behind it", "WiFi interference"], a: 1,
    ...
    { q: "cwnd vs rwnd — the real distinction?",
      opts: ["Two names for one value", "rwnd protects the receiver's buffer; cwnd is the sender's estimate protecting the network path — the effective limit is the minimum of both", "cwnd applies only to UDP"], a: 1,
      why: "Two independent brakes on one pedal. Either one alone can be your bottleneck." },
  ]},
},
{
  id: "n08", code: "N10", title: "Routing the Internet",
  layers: ["L3"], est: "~65 min",
  tag: "From 'which route wins' to OSPF, BGP, and how ~80,000 networks agree on a map with no one in charge.",
  lessons: [
    { id: "n08l1", title: "How a router chooses", est: "~10 min", blocks: [
    ...
    { id: "n08l2", title: "IGPs: mapping your own house", est: "~12 min", blocks: [
    ...
    { id: "n08l3", title: "BGP: policy, not shortest path", est: "~12 min", blocks: [
      ...
      { note: "BGP convergence is also why 'the internet is down' sometimes fixes itself in 90 seconds: routes are withdrawn, alternatives propagate AS by AS, the map heals. The global routing table sits at roughly a million IPv4 prefixes — every one an aggregation decision from N07.", label: "the living map" },
    ]},
    { id: "n08l4", title: "Redundancy patterns you'll actually meet", est: "~10 min", blocks: [
  ...
  exercises: [
    { id: "n08e1", type: "match", title: "Route selection referee", kind: "MATCH LAB",
      prompt: "A router hears about 10.20.30.0/24 (or contains it) from five sources. Match each candidate to its fate for a packet to 10.20.30.7.",
      pairs: [
        { t: "Connected interface: 10.20.30.0/24", d: "Wins — same prefix length, and administrative distance 0 beats every other source" },
        { t: "Static route: 10.20.30.0/24 via R2", d: "Loses on trust — AD 1 is beaten by connected's AD 0 for the identical prefix" },
        { t: "OSPF: 10.20.30.0/24, lowest cost path", d: "Loses on trust — right prefix, but AD ~110 ranks below connected and static" },
        { t: "BGP: 10.20.0.0/16", d: "Loses on specificity — shorter prefix never competes with a matching /24, regardless of source" },
        { t: "OSPF: 10.20.30.0/25 covering .0–.127", d: "Would win for .7 if present — longest prefix is evaluated before any trust comparison" },
      ],
  ...
  quiz: { id: "n08q", questions: [
    { q: "Why does BGP carry the full AS path with every route?",
      opts: ["For debugging only", "Loop prevention (reject paths containing your own AS) and raw material for policy decisions", "To compute geographic distance"], a: 1,
    ...
    { q: "An ISP prefers a longer path through a customer over a shorter path through a provider. Why?",
      opts: ["Misconfiguration", "Policy outranks path length — customer routes earn revenue, and LOCAL_PREF is consulted before AS_PATH", "BGP can't measure length"], a: 1,
      why: "The internet routes money first, hops second. 'Suboptimal' is often 'profitable.'" },
    { q: "What made classic BGP hijacks possible?",
      opts: ["Weak encryption", "BGP historically had no origin validation — routers believed any AS announcing any prefix", "IPv4 exhaustion"], a: 1,
    ...
    { q: "With ECMP across two equal links, why can a single large download only use one link's bandwidth?",
      opts: ["ECMP is broken", "Load-sharing hashes per-flow to prevent reordering — one 5-tuple sticks to one path", "TCP forbids multiple paths"], a: 1,
