...
export const NET_C = [
{
  id: "n10", code: "N12", title: "DNS Deep Dive",
  layers: ["L7"], est: "~65 min",
  tag: "The hierarchy, the resolution dance, the records that matter, and what securing names really buys.",
  lessons: [
    { id: "n10l1", title: "A distributed database with delegation", est: "~10 min", blocks: [
    ...
    { id: "n10l2", title: "The resolution dance", est: "~10 min", blocks: [
      ...
      { code: { lang: "sh", title: "watch the iteration yourself", body: "# +trace makes dig act like a recursive resolver, from root down:\ndig +trace www.example.com\n\n# query a specific server directly (bypass your resolver):\ndig @1.1.1.1 example.com A\n\n# read TTLs counting down in your resolver's cache:\ndig example.com   # run twice, compare the TTL column" } },
    ]},
    { id: "n10l3", title: "Records that earn their keep", est: "~12 min", blocks: [
    ...
    { id: "n10l4", title: "Securing names: two different problems", est: "~12 min", blocks: [
      ...
      { note: "Tunnel tie-in: encrypted DNS both helps and complicates you. A browser doing its own DoH quietly bypasses the tunnel's carefully-assigned resolver — split-DNS enterprise setups (S01) break, and your 'no leaks' story now includes a browser setting. Real clients detect and account for it.", label: "DoH vs your VPN" },
    ]},
  ],
  exercises: [
    { id: "n10e1", type: "order", title: "Cold-cache resolution", kind: "SEQUENCE LAB",
      prompt: "A recursive resolver with an empty cache is asked for www.example.com. Order the full dance.",
      items: [
    ...
    { id: "n10e2", type: "match", title: "Record type triage", kind: "MATCH LAB",
      prompt: "Match each situation to the record type it involves.",
      pairs: [
        { t: "Mail to the domain bounces; senders can't find a mail host", d: "MX — the mail-exchanger records are missing or wrong" },
        { t: "Provider says 'verify ownership by adding a string to DNS'", d: "TXT — the junk drawer that became critical infrastructure" },
        { t: "www works but example.com (apex) can't alias the CDN", d: "CNAME — forbidden at the apex, hence ALIAS/flattening workarounds" },
        { t: "A CA refuses to issue a certificate for the domain", d: "CAA — the record whitelisting which authorities may issue" },
        { t: "Your mail server's IP has no name when looked up backwards", d: "PTR — missing reverse DNS, a spam-score red flag" },
      ],
  ...
  quiz: { id: "n10q", questions: [
    { q: "You update an A record and half the world sees the change hours later. The mechanism?",
      opts: ["DNS servers sync on a schedule", "Caches worldwide hold the old answer until its TTL expires — 'propagation' is just caches counting down", "The registrar batches changes"], a: 1,
    ...
    { q: "DNSSEC vs DoH — the accurate division of labor?",
      opts: ["Both encrypt queries", "DNSSEC proves answers weren't forged; DoH hides queries from on-path observers — authenticity vs transport privacy", "DoH replaces DNSSEC"], a: 1,
    ...
    { q: "Why does a recursive resolver only rarely contact the root servers?",
      opts: ["Root servers rate-limit heavily", "Referral caching — one lookup teaches it the .com servers for days of future queries", "Roots only serve other roots"], a: 1,
    ...
    { q: "A user's browser has DoH enabled. What does that mean for their corporate VPN's split-DNS setup?",
      opts: ["Nothing — DoH follows system DNS settings", "Internal-name resolution can silently break: the browser ships queries to a public resolver, bypassing the tunnel's assigned one", "It makes split-DNS more reliable"], a: 1,
...
{
  id: "n11", code: "N13", title: "TLS, HTTP & QUIC",
  layers: ["L5","L6","L7"], est: "~70 min",
  tag: "The encryption the web actually runs on, the PKI behind it, and HTTP's three-act evolution into QUIC.",
  lessons: [
    { id: "n11l1", title: "TLS 1.3: the web's handshake", est: "~12 min", blocks: [
    ...
    { id: "n11l2", title: "PKI: manufactured trust", est: "~10 min", blocks: [
    ...
    { id: "n11l3", title: "HTTP's three acts", est: "~10 min", blocks: [
    ...
    { id: "n11l4", title: "QUIC: the transport rebuilt in userspace", est: "~12 min", blocks: [
    ...
    { id: "n11l5", title: "DTLS: TLS for datagrams", est: "~10 min", blocks: [
  ...
  exercises: [
    { id: "n11e1", type: "order", title: "TLS 1.3, cold start", kind: "SEQUENCE LAB",
      prompt: "A browser meets a server for the first time. Order the 1-RTT handshake to first request.",
      items: [
    ...
    { id: "n11e2", type: "match", title: "Which layer broke?", kind: "MATCH LAB",
      prompt: "Match each production symptom to the mechanism at fault.",
      pairs: [
        { t: "Site works in browsers, one legacy app rejects it", d: "Trust store — the app's CA bundle is stale and can't build the chain" },
        { t: "HTTP/2 site crawls on a 2% packet-loss link", d: "TCP head-of-line blocking — one loss stalls every multiplexed stream" },
        { t: "App breaks the day certificates rotate", d: "Certificate pinning without an escape hatch" },
        { t: "Censor blocks specific sites on a shared-IP host", d: "Plaintext SNI naming the target in the ClientHello — ECH's reason to exist" },
        { t: "Duplicate purchase from a resumed session", d: "0-RTT replay — early data must be idempotent, and this wasn't" },
      ],
  ...
  quiz: { id: "n11q", questions: [
    { q: "Why does TLS need certificates when WireGuard doesn't?",
      opts: ["TLS uses weaker crypto", "TLS authenticates strangers, so identity must be vouched mid-handshake; WireGuard pre-distributes peer keys in config", "Certificates are legally required"], a: 1,
    ...
    { q: "HTTP/2 solved head-of-line blocking, and HTTP/3 solved it again. Both true — how?",
      opts: ["Marketing repetition", "H2 fixed the application-level queue; one lost TCP packet still stalled all streams — H3's independent QUIC streams fixed the transport level", "H3 just renamed H2"], a: 1,
    ...
    { q: "What makes QUIC connections survive a Wi-Fi-to-LTE switch?",
      opts: ["Faster retransmission", "Connections are identified by connection ID rather than the address 5-tuple", "Carriers coordinate the handoff"], a: 1,
    ...
    { q: "Why is nearly all of QUIC encrypted, including headers TCP exposes?",
      opts: ["Pure privacy maximalism", "To prevent middlebox meddling and the protocol ossification that froze TCP's evolution", "To compress better"], a: 1,
    ...
    { q: "Why does DTLS need explicit sequence numbers, retransmission timers, and a cookie exchange when TLS has none of them?",
      opts: ["It predates good design", "It runs over UDP: no ordering to infer from, no reliable delivery for handshakes, and no TCP handshake to weakly validate source addresses", "For FIPS compliance"], a: 1,
