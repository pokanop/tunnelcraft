...
export const NET_C: Module[] = [
  {
    ...
    exercises: [
      ...
      },
      {
        id: "n10e_pcap",
        type: "pcap",
        title: "Cold cache, warm cache: a resolution in the wild",
        kind: "CAPTURE LAB",
        prompt:
          "Captured at a recursive resolver (`192.168.1.1`): a client asks for `example.com`, the resolver walks the hierarchy, then the **same question arrives again 30 seconds later**. Watch what the second resolution doesn't do.",
        why: "Root referral, TLD referral, authoritative answer, cache hit — the entire recursive/iterative split from lesson one, visible as ten packets. Caches are why the internet's phonebook doesn't melt.",
        packets: [
          {
            no: 1,
            time: "0.000000",
          ...
          },
          {
            ...
            dst: "198.41.0.4",
            proto: "DNS",
            info: "Standard query 0x8f31 A example.com",
          },
          {
            no: 3,
            time: "0.031200",
            ...
            dst: "192.168.1.1",
            proto: "DNS",
            info: "Standard query response 0x8f31 A example.com — no answer; authority: com NS a.gtld-servers.net (referral + glue)",
          },
          {
            no: 4,
            time: "0.031644",
            ...
            dst: "192.5.6.30",
            proto: "DNS",
            info: "Standard query 0x9c02 A example.com",
          },
          {
            no: 5,
            time: "0.058903",
            ...
            dst: "192.168.1.1",
            proto: "DNS",
            info: "Standard query response 0x9c02 A example.com — no answer; authority: example.com NS a.iana-servers.net (referral + glue)",
          },
          {
            no: 6,
            time: "0.059310",
            ...
            dst: "199.43.135.53",
            proto: "DNS",
            info: "Standard query 0xb744 A example.com",
          },
          {
            no: 7,
            time: "0.084288",
            ...
            dst: "192.168.1.1",
            proto: "DNS",
            info: "Standard query response 0xb744 A example.com A 93.184.216.34 (TTL 3600)",
          },
          {
            no: 8,
            time: "0.084701",
          ...
          },
          {
            ...
            dst: "192.168.1.1",
            proto: "DNS",
            info: "Standard query 0x77e9 A example.com",
          },
          {
            no: 10,
            time: "30.412301",
            ...
            dst: "192.168.1.20",
            proto: "DNS",
            info: "Standard query response 0x77e9 A example.com A 93.184.216.34",
          },
        ],
        questions: [
          {
            q: "Which response is served from cache, and what gives it away?",
            opts: [
              "Packet 8 — it's the last packet of the burst",
              "Packet 10 — the answer comes back in ~0.3 ms with zero upstream packets in between, versus ~85 ms and six upstream packets the first time",
              "Packet 3 — the root answered instantly",
            ],
            a: 1,
            why: "Two fingerprints of a cache hit: latency collapses (sub-millisecond vs a full hierarchy walk) and the resolver sends *nothing* upstream. The record lives in cache until its TTL (3600 s) counts down.",
          },
          {
            q: "What did the root server actually return in packet 3?",
            opts: [
              "The A record for example.com",
              "A referral — 'not my job, but the .com nameservers are over there' — with no answer section at all",
              "An NXDOMAIN error",
            ],
            a: 1,
            why: "Roots don't know example.com's address and never will. They delegate downward: authority records for `.com`, plus glue addresses. Every step of iteration is a better referral, not an answer.",
          },
          {
            q: "The client's query used transaction ID 0x1a2b. Which packet must carry the same ID?",
            opts: [
              "Packet 8 — the resolver's response to the client; IDs pair a response to its exact query, and the resolver mints fresh IDs (0x8f31, 0x9c02, 0xb744) for its own upstream questions",
              "Packet 2 — the resolver forwards the client's ID upstream",
              "All ten packets share one ID for the whole resolution",
            ],
            a: 0,
            why: "Each query/response pair is its own transaction with its own ID. That 16-bit match is also a (weak) anti-spoofing check — the reason cache-poisoning attacks race to guess it.",
          },
          {
            q: "Why does the second resolution (packets 9–10) involve so many fewer packets?",
            opts: [
              "The client asked a simpler question",
              "The resolver cached the answer from the first walk, so nothing upstream needs to be asked until the TTL expires",
              "The root server remembered the client",
            ],
            a: 1,
            why: "Recursive for you, iterative for itself, *cached for everyone*. One cold-cache walk pays the cost; every query for the next 3600 seconds rides free — that amortization is the design.",
          },
          {
            q: "In the cold-cache walk (packets 1–8), where does the actual A record first appear?",
            opts: [
              "Packet 3 — the root's response",
              "Packet 7 — the authoritative server (a.iana-servers.net) is the first to answer with A 93.184.216.34",
              "Packet 2 — the resolver's first upstream query",
            ],
            a: 1,
            why: "Packets 3 and 5 are referrals with empty answer sections. Only the zone's authoritative server speaks the record itself; packet 8 then relays it (and packet 10 replays it from cache).",
          },
        ],
      },
    ],
