...
export const NET_D = [
{
  id: "n12", code: "N16", title: "The Toolbelt",
  layers: ["L3","L4","L7"], est: "~65 min",
  tag: "ping, traceroute, ss, dig, tcpdump, curl, nmap — and a repeatable method that turns 'it's broken' into a layer.",
  lessons: [
    { id: "n12l1", title: "A method before the tools", est: "~10 min", blocks: [
    ...
    { id: "n12l2", title: "Reachability: ping, traceroute, mtr", est: "~10 min", blocks: [
    ...
    { id: "n12l3", title: "Sockets and captures: ss and tcpdump", est: "~12 min", blocks: [
    ...
    { id: "n12l4", title: "dig, curl, nmap: the application-layer bench", est: "~12 min", blocks: [
  ...
  exercises: [
    { id: "n12e1", type: "match", title: "Symptom → next command", kind: "MATCH LAB",
      prompt: "For each finding, pick the move a professional makes next.",
      pairs: [
        { t: "Ping 1.1.1.1 fine; ping example.com fails", d: "dig — it's a name-resolution problem, stop testing the network" },
        { t: "Service 'unreachable' — but is anything listening?", d: "ss -tlnp — check listeners and their bind addresses first" },
        { t: "mtr shows loss only at hop 7, none afterward", d: "Nothing — that's ICMP rate-limiting at one router, not path loss" },
        { t: "Two ends disagree about whether packets were sent", d: "tcpdump on both sides — let the wire arbitrate" },
        { t: "TLS 'sometimes fails' to one API", d: "curl -v — watch which handshake phase dies, cert details included" },
      ],
      why: "Tool selection is the skill. Each symptom already names its layer; the command merely confirms it." },
    { id: "n12e2", type: "ports", title: "Port reflexes — infinite drill", kind: "LIVE DRILL",
      prompt: "Reading ss, tcpdump, nmap, and firewall rules at speed requires port-number reflexes. Name the port for each service. New rounds forever." },
  ],
  quiz: { id: "n12q", questions: [
    { q: "A connect attempt returns 'Connection refused' instantly. What do you now know?",
      opts: ["A firewall blocked it", "The host is up and reachable — it answered with RST; nothing listens on that port", "DNS failed"], a: 1,
    ...
    { q: "Traceroute shows 200 ms at hop 4, then 20 ms at hops 5–10. Verdict?",
      opts: ["Hop 4 is congested and slowing traffic", "Hop 4's router deprioritizes generating ICMP — forwarding is fine, as the faster later hops prove", "The trace is corrupted"], a: 1,
    ...
    { q: "A web service works locally on its host but times out from the network. ss shows it LISTENing. Prime suspect?",
      opts: ["DNS misconfiguration", "Bound to 127.0.0.1 instead of 0.0.0.0 — or a host firewall dropping the port; ss's Local Address column decides which", "MTU too low"], a: 1,
    ...
    { q: "Why capture traffic at both endpoints of a failing connection?",
      opts: ["Twice the data is better", "To separate 'never sent' from 'sent but never arrived' — each capture answers a different half", "tcpdump requires it"], a: 1,
...
{
  id: "n13", code: "N17", title: "Security Fundamentals",
  layers: ["L2","L3","L4","L7"], est: "~65 min",
  tag: "Attacks on the wire, firewalls done right, segmentation and detection, DDoS — and why zero trust won.",
  lessons: [
    { id: "n13l1", title: "Attacks on the wire", est: "~10 min", blocks: [
    ...
    { id: "n13l2", title: "Firewalls, honestly", est: "~12 min", blocks: [
    ...
    { id: "n13l3", title: "Segmentation & detection", est: "~10 min", blocks: [
    ...
    { id: "n13l4", title: "DDoS & the zero-trust conclusion", est: "~12 min", blocks: [
      ...
      { note: "Notice what a zero-trust access client actually is: per-flow policy (S02's Flow dispatch), identity-keyed crypto (T02/T03), device posture as a live signal (S02's PostureProvider), fail-closed enforcement (S01), and flow telemetry (S03). The VPN client you're building *is* the zero-trust architecture's enforcement point. The fundamentals track and the engineering tracks were the same course all along.", label: "the whole map" },
    ]},
  ],
  exercises: [
    { id: "n13e1", type: "match", title: "Attack → the defense that answers its mechanism", kind: "MATCH LAB",
      prompt: "Match each attack to the control that addresses its mechanism, not just its symptoms.",
      pairs: [
        { t: "ARP spoofing on shared Wi-Fi", d: "End-to-end encryption — make owning L2 worthless (plus switch-level ARP inspection)" },
        { t: "SYN flood filling the half-open queue", d: "SYN cookies — answer statelessly, spend memory only on completed handshakes" },
        { t: "UDP reflection/amplification flood", d: "Anycast + scrubbing capacity; upstream, providers filtering spoofed sources" },
        { t: "Compromised IoT device probing the file server", d: "Segmentation — VLANs and inter-segment filtering to kill lateral movement" },
        { t: "Stolen laptop with a valid VPN key", d: "Zero-trust posture checks — device state re-evaluated continuously, not just identity at connect time" },
      ],
  ...
  quiz: { id: "n13q", questions: [
    { q: "\"We're safe from inbound attacks — we're behind NAT.\" The precise correction?",
      opts: ["Correct as stated", "The protection is the stateful only-replies-admitted filtering that accompanies NAT — NAT itself is address translation, not a security control", "NAT encrypts inbound traffic"], a: 1,
    ...
    { q: "Why did SYN cookies defeat SYN floods?",
      opts: ["They block the attacker's IP", "The server stops storing state for half-open connections — the 'cookie' encodes it in the sequence number, so only completed handshakes cost memory", "They rate-limit all SYNs"], a: 1,
    ...
    { q: "Traffic is increasingly encrypted. How does network detection stay useful?",
      opts: ["It can't — encryption ends detection", "Metadata and flow analysis: who talks to whom, when, how much — anomalies show in NetFlow regardless of payload encryption", "Decrypt everything at the perimeter"], a: 1,
    ...
    { q: "The core inversion zero trust makes versus the perimeter model?",
      opts: ["Stronger perimeter firewalls", "Network location stops implying trust — every access is verified against identity and device posture, continuously", "Blocking all remote work"], a: 1,
