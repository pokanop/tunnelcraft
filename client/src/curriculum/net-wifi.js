...
{
  id: "n03", code: "N03", title: "Wi-Fi & the Wireless Link",
  layers: ["L1","L2"], est: "~75 min",
  ...
  lessons: [
    { id: "n03l1", title: "Air is a shared, half-duplex medium", est: "~12 min", blocks: [
      ...
      { note: "Carry this into tunnel work: a VPN's throughput ceiling on Wi-Fi is set by airtime contention, not by your crypto. When a user reports 'the VPN is slow on Wi-Fi but fine on Ethernet,' the tunnel is usually innocent — they're contending for air with the neighbourhood. Latency and jitter (N02) also spike under contention, which matters far more for a tunnel than raw bandwidth.", label: "diagnosis instinct" },
    ]},
    { id: "n03l2", title: "CSMA/CA: taking turns without collisions", est: "~12 min", blocks: [
      ...
      { note: "All of this is *per-frame* overhead — sensing, backoff, ACKs, occasional RTS/CTS — layered on top of the actual data. It's why measured Wi-Fi throughput is routinely half the negotiated rate. The efficiency gap isn't a bug; it's the price of coordinating a shared medium with no central clock.", label: "why 866 becomes 400" },
    ]},
    { id: "n03l3", title: "The association dance: scan, auth, associate", est: "~13 min", blocks: [
      ...
      { note: "The frame types split into three families worth knowing by name: **management** (beacons, probes, auth, association, and — crucially — *deauthentication*), **control** (RTS/CTS, ACK), and **data** (your actual traffic, carrying the Ethernet-like payload from N02). The attacks in lesson 5 almost all abuse *management* frames, because in classic Wi-Fi those frames are unauthenticated.", label: "three frame families" },
    ]},
    { id: "n03l4", title: "Securing the link: WPA2, WPA3 & the 4-way handshake", est: "~14 min", blocks: [
  ...
  exercises: [
    { id: "n03e1", type: "match", title: "Wireless vocabulary lock-in", kind: "MATCH LAB",
      ...
      pairs: [
        ...
        { t: "CSMA/CA", d: "Listen-then-backoff medium access — avoidance, because a radio can't detect its own collisions" },
        { t: "4-way handshake", d: "The post-association exchange that derives fresh per-session keys from the master key" },
        { t: "PMF (802.11w)", d: "Protected management frames — authenticates deauth/disassoc, killing the deauth attack" },
        { t: "Evil twin", d: "A rogue AP impersonating a trusted SSID to become the victim's gateway" },
      ],
    ...
    { id: "n03e2", type: "order", title: "From cold radio to routed packet", kind: "SEQUENCE LAB",
      prompt: "A laptop with Wi-Fi off joins a WPA2 network and loads a page. Order the full sequence — note where L2 ends and the L3 rituals you already know begin.",
      items: [
  ...
  quiz: { id: "n03q", questions: [
    { q: "A single distant, slow client noticeably drags down throughput for everyone else on the same Wi-Fi channel. Why?",
      opts: ["Its weak signal corrupts other clients' frames", "The medium is shared and half-duplex — its low data rate makes each frame occupy the channel longer, stealing airtime from all", "The AP prioritizes distant clients"], a: 1,
    ...
    { q: "Why does Wi-Fi use collision *avoidance* (CSMA/CA) rather than the collision *detection* (CSMA/CD) of classic Ethernet?",
      opts: ["Avoidance is simply faster", "A transmitting radio is deaf to its own frequency and cannot detect a collision while it happens", "Detection requires a central controller Wi-Fi lacks"], a: 1,
    ...
    { q: "What does the WPA2/WPA3 4-way handshake accomplish, and when does it run?",
      opts: ["It carries the passphrase to the AP during scanning", "After association, it exchanges nonces to derive fresh per-session encryption keys from the shared master key without transmitting them", "It replaces DHCP for secured networks"], a: 1,
    ...
    { q: "WPA3's SAE is a meaningful upgrade over WPA2-Personal primarily because:",
      opts: ["It uses a longer AES key", "A captured handshake yields nothing to an offline brute-force attacker, and each session gains forward secrecy", "It hides the SSID"], a: 1,
    ...
    { q: "An attacker floods spoofed deauthentication frames at a victim. Which control specifically defeats this, and why does it work?",
      opts: ["A stronger passphrase — it re-secures the handshake", "PMF / 802.11w — it authenticates management frames, so forged deauths are rejected", "RTS/CTS — it reserves the air against the attacker"], a: 1,
