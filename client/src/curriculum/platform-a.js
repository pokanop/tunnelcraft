...
{
  id: "p01", code: "P01", title: "Apple Internals: Network Extension",
  layers: ["XP"], est: "~75 min",
  tag: "NEPacketTunnelProvider, utun, content filters, transparent proxies — the doors Apple leaves open, in detail.",
  lessons: [
    { id: "p01l1", title: "The Network Extension family", est: "~12 min", blocks: [
    ...
    { id: "p01l2", title: "Life as a packet tunnel provider", est: "~14 min", blocks: [
    ...
    { id: "p01l3", title: "utun, scoped routes & kill-switch semantics", est: "~12 min", blocks: [
    ...
    { id: "p01l4", title: "Content filters & the transparent proxy", est: "~12 min", blocks: [
  ...
  exercises: [
    { id: "p01e1", type: "match", title: "Pick the right provider class", kind: "MATCH LAB",
      prompt: "Match each product requirement to the Network Extension machinery that implements it.",
      pairs: [
        { t: "Full-device WireGuard VPN", d: "NEPacketTunnelProvider — L3 packets via packetFlow over a system utun" },
        { t: "Route only *.corp.example flows, touch nothing else", d: "NETransparentProxyProvider — claim flows via includedNetworkRules" },
        { t: "Block content, provably without exfiltrating traffic", d: "NEFilterDataProvider — verdicts from a sandbox with no network access" },
        { t: "Send every DNS query to your resolver", d: "NEDNSSettings with matchDomains = [\"\"] (or a DNS proxy provider)" },
        { t: "Auto-connect whenever the device leaves trusted Wi-Fi", d: "NEOnDemandRules on the saved tunnel configuration" },
      ],
  ...
  quiz: { id: "p01q", questions: [
    { q: "Your iOS tunnel dies under sustained throughput with no error in your logs. First suspect?",
      opts: ["Apple throttling VPNs", "The ~50 MB extension memory limit — the OS kills providers that exceed it; audit buffering and per-packet allocation", "Certificate expiry"], a: 1,
    ...
    { q: "How do you implement 'corporate domains resolve through the tunnel, everything else uses local DNS' on Apple?",
      opts: ["Rewrite /etc/resolv.conf from the extension", "NEDNSSettings.matchDomains listing the corporate domains — the system routes only those queries to your servers", "Block port 53 and hope"], a: 1,
    ...
    { q: "Why does the content-filter API use two separate providers?",
      opts: ["Performance isolation", "Privacy by construction: the data provider sees traffic but has no network access; the control provider has network access but sees no traffic", "Legacy compatibility"], a: 1,
    ...
    { q: "What does includeAllNetworks give you, and what does it cost?",
      opts: ["Faster routing at the cost of battery", "Kill-switch-like 'no traffic outside the tunnel' semantics — at the cost of fighting captive portals, which need pre-tunnel HTTP", "IPv6 support at the cost of IPv4"], a: 1,
...
{
  id: "p02", code: "P02", title: "Windows Internals: Wintun, WFP & the Route Table",
  layers: ["XP"], est: "~75 min",
  ...
  lessons: [
    { id: "p02l1", title: "Getting packets: TAP-Windows vs wintun", est: "~12 min", blocks: [
    ...
    { id: "p02l2", title: "Windows Filtering Platform: the rule engine", est: "~14 min", blocks: [
    ...
    { id: "p02l3", title: "Routing, metrics & the DNS leak machine", est: "~14 min", blocks: [
  ...
  exercises: [
    { id: "p02e1", type: "order", title: "Build the Windows kill switch", kind: "SEQUENCE LAB",
      prompt: "Order the steps that produce a fail-closed full tunnel on Windows (each depends on the previous).",
      items: [
    ...
    { id: "p02e2", type: "match", title: "Windows mechanism triage", kind: "MATCH LAB",
      prompt: "Match each symptom on a Windows client to its mechanism.",
      pairs: [
        { t: "VPN connected, but traffic still exits the Ethernet NIC", d: "Metric fight — the physical default route outranks the tunnel's; use lower metric or the /1 trick" },
        { t: "Routes perfect, yet internal hostnames resolve to public IPs", d: "Multi-homed DNS racing — the resolver asked the LAN server too; NRPT or block port 53 off-tunnel" },
        { t: "Daemon crashes and traffic flows in the clear", d: "Dynamic WFP filters died with the session — fail-closed requires persistent filters" },
        { t: "Tray icon says 'No Internet' while the tunnel works", d: "Kill switch is blocking NCSI connectivity probes" },
        { t: "Per-app rule ignores a UWP app", d: "ALE filters match by application path — packaged apps need package-id conditions instead" },
      ],
  ...
  quiz: { id: "p02q", questions: [
    { q: "Why does wintun outperform the legacy TAP driver?",
      opts: ["Newer compiler", "L3-only design plus shared-memory ring buffers — packets move without per-packet syscalls or fake Ethernet framing", "It runs in userspace entirely"], a: 1,
    ...
    { q: "What do 0.0.0.0/1 + 128.0.0.0/1 accomplish that replacing 0.0.0.0/0 doesn't?",
      opts: ["They're faster to look up", "They win by longest-prefix over any default route without deleting or restoring the original — cleaner teardown, no fights with other software", "They enable IPv6"], a: 1,
    ...
    { q: "A Windows kill switch must survive your process crashing. What's the load-bearing detail?",
      opts: ["A watchdog process", "Persistent (or boot-time) WFP filters — kernel-resident rules that outlive the session, unlike dynamic filters", "Running the UI as admin"], a: 1,
    ...
    { q: "Why can DNS leak on Windows even with a perfect route table?",
      opts: ["It can't", "The resolver is multi-homed and can query every interface's servers in parallel, taking the fastest answer — names escape via the LAN NIC", "IPv6 is enabled"], a: 1,
