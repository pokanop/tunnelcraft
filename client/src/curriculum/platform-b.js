...
export const PLATFORM_B = [
{
  id: "p03", code: "P03", title: "Linux & Android Internals",
  layers: ["XP"], est: "~70 min",
  tag: "/dev/net/tun for real, policy routing dissected line by line, and VpnService — the fd Android hands your Rust core.",
  lessons: [
    { id: "p03l1", title: "/dev/net/tun, properly", est: "~12 min", blocks: [
    ...
    { id: "p03l2", title: "Policy routing: the WireGuard incantation, dissected", est: "~14 min", blocks: [
    ...
    ]},    { id: "p03l3", title: "Android: VpnService & the borrowed fd", est: "~14 min", blocks: [
  ...
  exercises: [
    { id: "p03e1", type: "blank", title: "Read the policy-routing incantation", kind: "CODE LAB",
      prompt: "Fill the blanks so this Linux full-tunnel setup routes everything through wg0 while the encrypted outer packets and LAN traffic escape correctly.",
    ...
    { id: "p03e2", type: "match", title: "Same job, three platforms", kind: "MATCH LAB",
      prompt: "Match each cross-platform job to its Linux/Android primitive (P01/P02 taught the Apple/Windows versions).",
      pairs: [
        { t: "Keep the outer UDP socket out of the tunnel", d: "fwmark rule on Linux; VpnService.protect() on Android" },
        { t: "Per-app split tunneling", d: "addAllowed/DisallowedApplication — first-class in the VpnService Builder" },
        { t: "Fail-closed kill switch", d: "nftables default-drop on Linux; system always-on + lockdown on Android" },
        { t: "Split DNS by domain", d: "resolvectl domain wg0 ~corp.example via systemd-resolved" },
        { t: "Zero-drop config change", d: "Establish a second tun fd, migrate, close the old (Android seamless handover)" },
      ],
  ...
  quiz: { id: "p03q", questions: [
    { q: "What does 'ip rule add table main suppress_prefixlength 0' accomplish in the wg-quick setup?",
      opts: ["Deletes the main table's default route", "Lets main's specific routes (LAN, printers) keep winning while ignoring only its default — LAN exclusion without enumerating LANs", "Suppresses IPv6"], a: 1,
    ...
    { q: "Why must an Android VPN call protect() on its own UDP socket?",
      opts: ["Battery optimization", "Otherwise the encrypted outer packets are captured by the VPN's own routes — the classic tunnel-swallows-itself loop", "TLS requires it"], a: 1,
    ...
    { q: "The strongest kill switch on Android is…",
      opts: ["An in-app nftables rule", "System always-on VPN with lockdown — the OS itself blocks traffic when your VPN is down, surviving your process entirely", "A watchdog thread"], a: 1,
    ...
    { q: "What makes TUNSETOFFLOAD/GSO through the tun device such a large speedup?",
      opts: ["It compresses packets", "The kernel batches many segments into one giant buffer per syscall, so your read/encrypt loop runs per-batch instead of per-MTU-packet", "It bypasses encryption"], a: 1,
  ...
  id: "p04", code: "P04", title: "Proxies, PAC & Split-Tunnel Strategy",
  layers: ["L4","L7","XP"], est: "~65 min",
  tag: "CONNECT, SOCKS5, PAC files and WPAD, and the four ways to split a tunnel — the traffic-steering layer above routing.",
  lessons: [
    { id: "p04l1", title: "The proxy bestiary", est: "~12 min", blocks: [
    ...
    { id: "p04l2", title: "PAC & WPAD: proxy policy as JavaScript", est: "~13 min", blocks: [
    ...
    { id: "p04l3", title: "Split tunneling: the strategy matrix", est: "~14 min", blocks: [
  ...
  exercises: [
    { id: "p04e1", type: "match", title: "Steer this traffic", kind: "MATCH LAB",
      prompt: "Match each requirement to the steering mechanism that fits it best.",
      pairs: [
        { t: "Only 10.0.0.0/8 through the corporate tunnel", d: "Route-based split: AllowedIPs / includedRoutes with the RFC 1918 block" },
        { t: "Slack and Jira via VPN, Spotify direct, on Android", d: "Per-app: addAllowedApplication on the VpnService Builder" },
        { t: "*.corp.example steered without knowing its CDN's IPs", d: "Domain-based: split DNS plus resolver-synthesized addresses in a captured range" },
        { t: "HTTPS out of a locked-down office network", d: "HTTP CONNECT through the mandated proxy — per the PAC verdict" },
        { t: "Curl through the tunnel daemon without touching routes", d: "Local SOCKS5 listener exposed by the client (socks5h to keep DNS inside)" },
      ],
  ...
  quiz: { id: "p04q", questions: [
    { q: "How does an HTTPS connection remain end-to-end encrypted through a corporate HTTP proxy?",
      opts: ["The proxy re-encrypts it", "CONNECT opens a blind TCP relay; TLS then runs client-to-origin through the pipe — the proxy sees the target name, not the content", "It doesn't"], a: 1,
    ...
    { q: "Why is isInNet() in a PAC file a leak concern for a VPN client?",
      opts: ["It's slow", "It performs a DNS resolution during PAC evaluation — a query that may escape before the tunnel is up and outside your controlled resolver path", "It only supports IPv4"], a: 1,
    ...
    { q: "What made WPAD a classic attack vector?",
      opts: ["Weak encryption", "Auto-discovery trusts the local network: whoever answers for 'wpad' (DHCP or DNS) hands every client its proxy config — instant on-path position", "PAC files are compiled"], a: 1,
    ...
    { q: "Domain-based split tunneling is cleanly configured, yet corporate names sometimes resolve publicly on some machines. Likely cause?",
      opts: ["The TTLs are too long", "Browsers using their own DoH bypass the OS resolver that implements the split — the N12 failure mode in production", "PTR records are missing"], a: 1,
