...
export const RUST_EXTRA = [
{
  id: "r04", code: "R05", title: "The Professional Rust Toolbelt",
  layers: ["RS"], est: "~55 min",
  tag: "Workspaces, the testing pyramid, lints and CI gates, and the tracing/profiling stack — shipping-grade habits.",
  lessons: [
    { id: "r04l1", title: "Workspaces & features", est: "~12 min", blocks: [
    ...
    { id: "r04l2", title: "The testing pyramid, Rust edition", est: "~12 min", blocks: [
      ...
      { code: { lang: "rust", title: "paused time: testing timers without waiting", body: "#[tokio::test(start_paused = true)]\nasync fn rekey_fires_at_120s() {\n    let mut peer = test_peer();\n    tokio::time::advance(Duration::from_secs(121)).await;\n    let out = peer.tunn_update(&mut buf);\n    assert!(matches!(out, TunnResult::WriteToNetwork(_)), \"expected handshake initiation\");\n}" } },
    ]},
    { id: "r04l3", title: "Lints, audits & CI gates", est: "~12 min", blocks: [
    ...
    { id: "r04l4", title: "Seeing inside: tracing & profiling", est: "~12 min", blocks: [
  ...
  exercises: [
    { id: "r04e1", type: "blank", title: "Wire up a timer test", kind: "CODE LAB",
      prompt: "Complete this test so it verifies keepalive behavior instantly instead of waiting 25 real seconds.",
  ...
  quiz: { id: "r04q", questions: [
    { q: "Why should cargo features be additive?",
      opts: ["Cargo refuses subtractive names", "Feature unification: cargo merges the features every dependent requests, so a feature that removes behavior breaks whichever crate didn't ask for it", "Additive features compile faster"], a: 1,
    ...
    { q: "What can integration tests in tests/ verify that unit tests structurally cannot?",
      opts: ["Async code", "That your public API alone supports real workflows — they compile as external crates with no private access", "Performance"], a: 1,
    ...
    { q: "tokio-console shows one task's poll running for 80 ms. The diagnosis?",
      opts: ["The task is high priority", "Something inside it blocks synchronously — a poll should return in microseconds, and a long poll starves every task on that worker", "Normal for network tasks"], a: 1,
