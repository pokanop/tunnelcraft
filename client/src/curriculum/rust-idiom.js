...
{
  id: "r05", code: "R02", title: "Idiomatic Rust: Types, Errors & APIs",
  ...
  lessons: [
    { id: "r05l1", title: "Make invalid states unrepresentable", est: "~12 min", blocks: [
    ...
    { id: "r05l2", title: "Conversions & trait etiquette", est: "~12 min", blocks: [
    ...
    { id: "r05l3", title: "Error craft", est: "~12 min", blocks: [
    ...
    { id: "r05l4", title: "Iterators, slices & zero-cost habits", est: "~12 min", blocks: [
  ...
  exercises: [
    { id: "r05e1", type: "blank", title: "Parse hostile bytes idiomatically", kind: "CODE LAB",
      prompt: "Complete this wire-format parser so it uses the standard fallible-conversion trait, structured errors, and slice carving — no panics reachable from network input.",
    ...
    { id: "r05e2", type: "match", title: "Idiom or anti-pattern?", kind: "MATCH LAB",
      prompt: "Match each API decision to the professional verdict.",
      pairs: [
        { t: "fn seal(&mut self, buf: &mut [u8])", d: "Idiomatic hot path — caller controls allocation, buffers get reused" },
        { t: "struct Conn { open: bool, ready: bool }", d: "Anti-pattern — boolean soup encodes impossible states; wants an enum" },
        { t: "unwrap() on a packet parse in the data path", d: "Anti-pattern — hostile input is weather, not a bug; return a Result" },
        { t: "impl Debug for PrivateKey that prints ****", d: "Idiomatic — derive everywhere except secrets, which redact by hand" },
        { t: "pub fn connect(s: String, p: u16, r: u16, t: u64)", d: "Anti-pattern — positional primitive soup; wants newtypes and a builder" },
      ],
  ...
  quiz: { id: "r05q", questions: [
    { q: "Why prefer enum Conn { Idle, Established { key: SessionKey } } over struct Conn { connected: bool, key: Option<SessionKey> }?",
      opts: ["Enums use less memory", "The enum makes 'connected but no key' unrepresentable — match forces handling of exactly the real states", "Option is slow"], a: 1,
    ...
    { q: "Your library's callers need to retry on timeout but surface config rejections. What does that requirement dictate?",
      opts: ["Log messages with different words", "A structured error enum (thiserror-style) with distinct variants callers can match on — not a string or one opaque error", "Return error codes as integers"], a: 1,
    ...
    { q: "When is unwrap() acceptable in production networking code?",
      opts: ["Whenever you're confident", "Only for invariants the surrounding code has just established — better written as expect(\"why\") — and never on values derived from network input", "Never, under any circumstances"], a: 1,
    ...
    { q: "A function returns Vec<u8> per packet and profiling shows allocation dominating. The idiomatic fix?",
      opts: ["Switch to unsafe pointers", "Change the signature to write into a caller-provided &mut [u8] so buffers can be reused across packets", "Add more threads"], a: 1,
