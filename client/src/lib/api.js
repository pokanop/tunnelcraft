...
export const api = {
  ...
  register: (email, password, displayName) =>
    req("/auth/register", { method: "POST", body: { email, password, displayName } }),
  ...
  resetPassword: (token, newPassword) =>
    req("/auth/reset-password", { method: "POST", body: { token, newPassword } }),
  deleteAccount: (password) =>
    req("/account", { method: "DELETE", body: { password, confirm: "DELETE" } }),
...
export function mergeProgress(a, b) {
  ...
  for (const [k, v] of Object.entries(Object.assign({}, ma.finals || {}, mb.finals || {}))) {
    out.meta.finals[k] = Math.max(
      (ma.finals && ma.finals[k]) || 0,
      (mb.finals && mb.finals[k]) || 0,
      v
    );
  }
  return out;
...
export function loadLocal() {
  try {
    return { ...EMPTY_PROGRESS, ...(JSON.parse(localStorage.getItem(GUEST_KEY)) || {}) };
  } catch {
