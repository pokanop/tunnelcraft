...
export function destroySession(secret) {
  q.deleteSession.run(sha256(secret));
}

export function validateSession(secret) {
...
export function makeAuth(userById) {
  return (req, res, next) => {
...
export function consumeVerifyToken(secret) {
...
export function consumeResetToken(secret) {
